...
class ActionLayer(BaseActionLayer):
    ...
    def _inconsistent_effects(self, actionA, actionB):
        ...
        return any([~e in actionA.effects for e in actionB.effects] or [~e in actionB.effects for e in actionA.effects])
...
class PlanningGraph:
    ...
    def h_levelsum(self):
        ...
        cost = 0
        for i in self.goal:
            for ids,layers in enumerate(self.literal_layers):
                if i in layers:
                    cost+= ids
                    break
        return cost
    ...
    def h_maxlevel(self):
        ...
        sol = 0
        for i in self.goal:
            for ids,layer in enumerate(self.literal_layers):
                if i in layer:
                    sol = max(sol,ids)
                    break
        return sol
    ...
    def h_setlevel(self):
        ...
        def NoMutex(layer):
            ...
            for i,j in combinations(self.goal,2):
                if layer.is_mutex(i,j):
                    return False
            return True
